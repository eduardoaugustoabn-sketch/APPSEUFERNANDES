...
| `linhaExpediente`: os 2 `<input type="time">` nativos (`className="border rounded px-2 py-1 bg-input disabled:opacity-50"`) | Viram `<Input type="time" className="w-28">`, mantendo `value`/`onChange`/`disabled`. O componente `Input` já trata `disabled` no seu próprio estilo (`disabled:opacity-50` já faz parte da classe base do `Input`), então a classe extra não precisa ser repetida — mas a largura explícita (`w-28`) é necessária pela mesma razão do `Select` de plano acima: o container dessa linha (`flex items-center gap-3`, dentro de `linhaExpediente`) **não** tem `flex-wrap`, então um `Input` `w-full` sem largura própria briga por espaço via `flex-shrink` em vez de manter a largura compacta original. |
